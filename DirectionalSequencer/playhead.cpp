...
void Playhead::InjectDependencies(size_t idx, const PlayheadDependencies& dependencies) {
	Index = idx;
...
void Playhead::Reset() {
	...
	auto dir = Dependencies.StepData->GetAdjustedCellValue(InitialStep.x, InitialStep.y, CellDataType::Direction);
	dir = (dir == 0 ? 3 : dir);
	Direction = dir;
	RepeatCount = 0;
	IsRepeat = false;
...
void Playhead::ProcessClockTrigger() {
	...
	LastClockTriggerTime = Dependencies.Timer->TotalMs;
	...
	if (LastClock > 0) {
		...
		auto newRate = Dependencies.Timer->TotalMs - LastClock;
		bool wayOff = newRate > (ClockRate * 10);
		if (StableClock && wayOff) {
			// don't change the clock rate
		} else {
			ClockRate = Dependencies.Timer->TotalMs - LastClock;
		}
	}
	...
	LastClock = Dependencies.Timer->TotalMs;
...
void Playhead::Process() {
	...
	if (Config.ResetWhenInactive && !InactiveResetApplied) {
		auto inactiveFor = Dependencies.Timer->TotalMs - LastClockTriggerTime;
	...
	if (Ratchets.Active) {
		if (Ratchets.RemainingDuration > 0) {
			...
			if (Ratchets.Count <= 0) {
				...
				GateStart = Dependencies.Timer->TotalMs;
	...
	float gate = ((Dependencies.Timer->TotalMs >= GateStart) && (Dependencies.Timer->TotalMs < GateEnd)) ? veloGate : GateLow;
...
void Playhead::ProcessTies() {
	if (TieCount == 0) {
		...
		if (EmitGate) {
			TieCount = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::TieSteps);
...
void Playhead::MoveToNextCell() {
	...
	if (CurrentStep.x == -1 && CurrentStep.y == -1) {
		...
		uint8_t dir = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Direction);
		if (dir > 0) {
			Direction = dir;
		} else if (Direction == 0) {
			Direction = 3;
...
void Playhead::CalculateStepValue() {
	...
	StepVal = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Value);
}
...
void Playhead::ProcessAccumulator() {
	...
	auto accumAdd = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::AccumAdd);
	auto accumTimes = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::AccumTimes);
...
void Playhead::ProcessDrift() {
	...
	auto driftProb = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::DriftProb);
	if (Dependencies.Random->Next(1, 100) <= driftProb) {
		...
		auto driftScale = static_cast<float>(Dependencies.Random->Next(0, res)) / res;
		driftScale = sqrt(driftScale);
		// calculate the drift range from the max
		auto driftRange = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::MaxDrift);
		// scale the drift range to get the actual drift, and make it negative half of the time
		auto actualDrift = driftRange * driftScale * (Dependencies.Random->Next(0, 1) == 1 ? -1 : 1);
		StepVal += actualDrift;
	}
}
...
void Playhead::ProcessMute() {
	auto mute = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Mute);
	if (mute) {
		EmitGate = false;
	}
}
...
void Playhead::ProcessRest() {
	...
	uint8_t restAfter = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::RestAfter);
	if (restAfter > 0) {
...
void Playhead::ProcessProbability() {
	...
	auto prob = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Probability);
	if (Dependencies.Random->Next(1, 100) > prob) {
		EmitGate = false;
	}
}
...
void Playhead::ProcessRepeats() {
	...
	if (!IsRepeat) {
		RepeatCount = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Repeats);
		if (Tie != TieMode::None)
...
void Playhead::CalculateGateLength() {
	GatePct = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::GateLength);
...
void Playhead::ProcessGlide() {
	...
	auto glidePct = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Glide);
	if (glidePct > 0) {
...
void Playhead::ProcessRatchets() {
	...
	auto ratchets = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Ratchets);
...
void Playhead::HumanizeGate() {
	...
	if (GatePct < 100) {
		...
		human *= 1000;
		float pct1 = Dependencies.Random->Next(0, human) / 1000.0f;
		float pct2 = Dependencies.Random->Next(0, human) / 1000.0f;
		...
		off2 = off2 * (Dependencies.Random->Next(0, 1) == 1 ? -1 : 1);
		GateEnd -= off2;
	}
}
...
void Playhead::CalculateVelocity() {
	...
	auto velo = Dependencies.StepData->GetAdjustedCellValue(CurrentStep.x, CurrentStep.y, CellDataType::Velocity);
...
void Playhead::HumanizeVelocity() {
	...
	auto human = Config.HumanizeValue;
	human *= 1000;
	float pct = Dependencies.Random->Next(0, human) / 1000.0f;
	auto off = Velocity * pct / 100.0f;
	// we can move up or down in velocity
	off = off * (Dependencies.Random->Next(0, 1) == 1 ? -1 : 1);
